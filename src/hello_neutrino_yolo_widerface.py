...
from deeplite_torch_zoo.wrappers.eval import yolo_eval_wider_face
...
class YOLOEval(TorchEvaluationFunction):
    ...
    def _compute_inference(self, model, data_loader, **kwargs):
        # silent **kwargs
        return yolo_eval_wider_face(model=model, data_root=self.data_root, net=self.net)


class YOLOLoss(LossFunction):
    def __init__(self, num_classes=8, device='cuda'):
...
if __name__ == '__main__':
    ...
    # model/dataset args
    parser.add_argument('--data_root', default='/neutrino/datasets/widerface/')
    ...
    parser.add_argument('-a', '--arch', metavar='ARCH', default='yolo5l', help='model architecture', choices=['yolo5l', 'yolo5m'])
    parser.add_argument('-c', '--num_classes', type=int, metavar='N', default=8, help='number of classes')
    # neutrino args
    ...
    reference_model = get_model_by_name(model_name=args.arch,
                                        dataset_name=f'wider_face_{args.num_classes}',
                                        pretrained=True,
                                        progress=True,
                                        device=device_map[args.device],)
