...
from .yolo_utils.general import init_seeds, print_args, set_logging, one_cycle, \
    colorstr, strip_optimizer, AverageMeter
from .yolo_utils.torch_utils import EarlyStopping, ModelEMA, de_parallel, select_device
...
def train(opt, model=None, dataset_splits=None, fine_tuning=False):
    ...
    if dataset_splits is None:
        print("Train script is loading its own data (was None in args)")
    ...
    # Model
    if model is None:
        print("Train script is loading its own model (was None in args)")
        model = create_model(
            model_name=opt.model_name,
    ...
    eval_function = get_eval_function(dataset_name=opt.dataset_type, model_name=opt.model_name)
    ...
    for epoch in range(start_epoch, epochs):  # epoch
        ...
        if RANK in [-1, 0]:
            ...
            if not noval and epoch % opt.eval_freq == 0:  # Calculate mAP
    ...
    if RANK in [-1, 0]:
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    # LOGGER.info(f'\nValidating {f}...')
                    LOGGER.info('returning best eval model')
                    test_set = opt.img_dir
                    gt = None
                    if opt.dataset_type in ("voc", "voc07"):
                        test_set = opt.img_dir / "VOC2007"
                    ...
                    if 'ema' in ckpt:
                        LOGGER.info('reloading ema from checkpoint')
                    model = ckpt['ema' if ckpt.get('ema') else 'model']

                    # some part of the code disables the grad which can be harmful
    ...
    return de_parallel(model) #, dataset_splits
...
def parse_opt():
    ...
    parser.add_argument(
        "--pretraining_source_dataset", type=str, default="coco",
